...
`Linux` 에서는 최신 버전을 제공하지 않는 경우가 많지만 오픈 소스 `PyMOL` 을 위한 바이너리 패키지를 기본 제공하기 때문에 아주 편리하게 설치 할 수 있습니다. 아래는 일부 인기 있는 배포판에 대한 설치 명령어입니다. 참고로 모든 명령은 `superuser` 로 실행해야 합니다.
...
`Homebrew` 는 macOS 와 Linux 시스템을 위한 패키지 관리자입니다. `Homebrew` 는 전용 디렉토리에 패키지를 설치하기 때문에 시스템의 다른 부분에 영향을 주지 않는 장점이 있습니다. 저는 가능하다면 이 방법을 가장 추천합니다. `Homebrew` 를 설치하는 방법은 다음과 같습니다:
...
앞서 살펴본 `Homebrew` 를 사용하는 방법과 동일하게 설치할 수 있습니다.
